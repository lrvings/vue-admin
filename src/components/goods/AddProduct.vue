<template>
  <div>
     <breadcrumb><slot slot="left">商品管理</slot><slot slot="right">添加商品</slot></breadcrumb>
      <el-card>
        <el-alert title="添加商品信息" type="info" center :closable="false"></el-alert>

        <el-steps :space="200" :active="+activeIndex" finish-status="success" align-center style="margin-top: 15px">
...
export default {
  ...
  data() {
    return {
      ...
      // 图片上传组件的请求对象
      headerObj: {
        Authorization: window.sessionStorage.getItem('tokenid')
      },
      picPreviewPath: '',
